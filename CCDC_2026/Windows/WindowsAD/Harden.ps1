<# Defend-AD-CCDC-Fixed.ps1
   Production-ready CCDC AD defense script with all critical bugs fixed
   
   FIXES APPLIED:
   - Fixed $ctx scoping bug (now using $script:ctx consistently)
   - Fixed Backup-GPO recursion (renamed to Backup-GpoObject)
   - Safer firewall defaults (removed high-risk ports, added UDP/53)
   - Sanitized GPO hash filenames
   - Monitor de-duplication to prevent event spam
   - Removed plaintext password logging
   - Added Safe Mode option
   - Better WinRM handling for NTP
   - Removed PlaceholderText for compatibility
   - Added execution policy bypass to scheduled task
#>
...
$LogFile     = Join-Path $BaseDir 'Fixed.log'
...
function Log([string]$m, [string]$level="INFO"){
  ...
  $logMsg = "$ts [$level] $m"
  $logMsg | Tee-Object -FilePath $LogFile -Append | Out-Host
  if ($script:txtLog) {
...
# --- Pre-flight Checks (FIXED: uses $script:ctx) ---
function Test-Prerequisites {
    ...
    if (-not $script:HasAD) {
        $issues += "ActiveDirectory module not available"
    }
    
    if (-not $script:HasGP) {
        $issues += "GroupPolicy module not available"
    }
    ...
    if ($script:HasAD -and $script:ctx.PreferredDC) {
        try {
            $null = Get-ADDomain -Server $script:ctx.PreferredDC -ErrorAction Stop
        } catch {
            $issues += "Cannot connect to DC: $($script:ctx.PreferredDC)"
        }
    }
...
function Backup-GpoObject {
    ...
    try {
        ...
        if ($gpo) {
            # Now calls the actual cmdlet (not our function)
...
function Invoke-LDAPHardening {
    ...
    if (-not $script:ctx.IsDC) {
        Log "LDAP: This is not a DC. LDAP hardening should be done on DCs only." "WARN"
    ...
    Log "LDAP: Starting hardening (will require DC reboot)..." "INFO"
    ...
    try {
        ...
        Log "LDAP: Set LDAPServerIntegrity=2 (require signing)" "SUCCESS"
        
        New-ItemProperty 'HKLM:\SYSTEM\CurrentControlSet\Services\NTDS\Parameters' `
            -Name LdapEnforceChannelBinding -Value 2 -PropertyType DWord -Force | Out-Null
        Log "LDAP: Set LdapEnforceChannelBinding=2 (always enforce)" "SUCCESS"
        
        New-ItemProperty 'HKLM:\SYSTEM\CurrentControlSet\Services\NTDS\Parameters' `
            -Name LDAPServerRequireStrongAuth -Value 2 -PropertyType DWord -Force | Out-Null
        Log "LDAP: Set LDAPServerRequireStrongAuth=2 (no cleartext)" "SUCCESS"
...
function Show-LDAPStatus {
    try {
        ...
        if ($signing -eq 2 -and $binding -eq 2 -and $strong -eq 2) {
            Log "  Status: HARDENED" "SUCCESS"
        } else {
            Log "  Status: NOT HARDENED" "WARN"
        }
    } catch {
...
# --- Core Security GPO (Now separate from one-click) ---
function Deploy-CoreSec-GPO {
    ...
    try{
        ...
        if(-not $gpo){ 
            ...
            Backup-GpoObject $gName
        }

        Set-GPRegistryValue -Name $gName `
          -Key "HKLM\SYSTEM\CurrentControlSet\Control\Lsa" `
          -ValueName "LmCompatibilityLevel" -Type DWord -Value 5 -Server $script:ctx.PreferredDC
        Log "CoreSec: Set LmCompatibilityLevel=5 (NTLMv2 only)" "SUCCESS"

        Set-GPRegistryValue -Name $gName `
          -Key "HKLM\SYSTEM\CurrentControlSet\Services\LanmanWorkstation\Parameters" `
          -ValueName "RequireSecuritySignature" -Type DWord -Value 1 -Server $script:ctx.PreferredDC
        Log "CoreSec: Enabled SMB client signing" "SUCCESS"
        
        Set-GPRegistryValue -Name $gName `
          -Key "HKLM\SYSTEM\CurrentControlSet\Services\LanmanServer\Parameters" `
          -ValueName "RequireSecuritySignature" -Type DWord -Value 1 -Server $script:ctx.PreferredDC
        Log "CoreSec: Enabled SMB server signing" "SUCCESS"
        
        Set-GPRegistryValue -Name $gName `
          -Key "HKLM\SYSTEM\CurrentControlSet\Control\Lsa" `
          -ValueName "NoLMHash" -Type DWord -Value 1 -Server $script:ctx.PreferredDC
        Log "CoreSec: Disabled LM hash storage" "SUCCESS"
...
function Harden-DNS {
    ...
    try{
        ...
        Set-DnsServerScavenging -ScavengingState $true `
            ...
            -ApplyOnAllZones $false -ErrorAction SilentlyContinue
...
function Invoke-SafeMode {
    if (-not (Test-Prerequisites)) {
        [System.Windows.Forms.MessageBox]::Show(
            "Pre-flight checks failed. Review the log for details.",
            "Error",
            [System.Windows.Forms.MessageBoxButtons]::OK,
    ...
    # Only the safest changes
    Harden-DNS
    Enable-Auditing
    
    Log "=== Safe mode complete ===" "SUCCESS"
    Log "Safe mode enabled: DNS hardening + Auditing + Guest disabled" "INFO"
}
        ...
        try{
            ...
            foreach($g in $gpos){
                try{
                    ...
                    Set-Content (Join-Path $hdir "$safeName.sha256") $hs
                }catch{}
            }
...
# --- Enhanced Monitor (FIXED: de-duplication) ---
function Start-PersistentMonitor {
    ...
    # FIXED: De-duplication state tracking
    $monScript = @"
`$BaseDir = '$BaseDir'
...
foreach(`$group in `$files.Keys) {
    try {
        `$baseline = @()
        if(Test-Path `$files[`$group]) {
            `$baseline = (Import-CliXml `$files[`$group]).SamAccountName
        }
...
# FIXED: Only log NEW events since last check
...
try {
    ...
    # Save state
    @{LastRecordId=`$lastRecordId} | Export-CliXml `$StateFile -Force
} catch {}
"@
    
    $monScriptPath = Join-Path $BaseDir 'monitor_script.ps1'
    Set-Content $monScriptPath $monScript
    
    try {
        # FIXED: Added ExecutionPolicy Bypass
...
# --- NTP (FIXED: Better WinRM handling) ---
function Set-NTP {
    if(-not $script:ctx.IsDomainJoined -or -not $script:HasAD){ 
...
# --- Firewall GPO (FIXED: Safe defaults + UDP/53 + optional admin ports) ---
function Push-FirewallRule-GPO {
    ...
    try{
        ...
        # FIXED: UDP ports (especially DNS)
        ...
        # Optional admin ports with subnet restriction
...
# --- Incident Response (FIXED: No password logging) ---
function Show-IncidentResponse {
    ...
    $btnDisable.Add_Click({
        if ($txtUser.Text) {
            try {
                ...
                Log "IR: Failed to disable user: $($_.Exception.Message)" "ERROR"
    ...
    $btnReset.Add_Click({
        if ($txtUser.Text) {
            try {
                $newPwd = -join ((65..90) + (97..122) + (48..57) + (33,35,36,37,38,42,43) | Get-Random -Count 16 | % {[char]$_})
                ...
                Log "IR: Failed to reset password: $($_.Exception.Message)" "ERROR"
                [System.Windows.Forms.MessageBox]::Show("Failed: $($_.Exception.Message)", "Error") | Out-Null
            }
        }
    })
    ...
    $btnExportLogs.Add_Click({
        try {
            ...
            Log "IR: Failed to export logs: $($_.Exception.Message)" "ERROR"
        }
    })
    ...
    $btnKillSessions.Add_Click({
        try {
            ...
            Log "IR: Failed to kill RDP sessions: $($_.Exception.Message)" "ERROR"
        }
    })
